...
public class GisUtil implements AutoCloseable {
     ...
     * 执行自适应分层聚类优化
     * 
     * <p>根据数据密度自适应调整分层策略，解决空间分块聚类中数据分布不均的问题：</p>
     * <ul>
     *   <li>第一层：智能密度检测，识别高密度热点区域</li>
     *   <li>第二层：高密度区域采用空间抽样+局部聚类</li>
     *   <li>第三层：低密度区域保持原始DBSCAN算法</li>
     *   <li>第四层：基于空间索引的快速聚类合并</li>
     * </ul>
     * 
     * @param points 待聚类的轨迹点列表
     * @param eps DBSCAN聚类半径参数
     * @param minPts DBSCAN最小点数参数
     * @return 聚类结果列表
     */
    private List<Cluster<TrackPoint>> performAdaptiveHierarchicalClustering(List<TrackPoint> points, double eps,
            int minPts) {
        if (points.size() < minPts) {
            return new ArrayList<>();
        }

        log.debug("开始自适应分层聚类，共 {} 个点，eps={}米，minPts={}", points.size(), eps, minPts);

        // 第一步：快速密度分析，识别高密度区域
        DensityAnalysisResult densityResult = analyzeDensityDistribution(points, eps, minPts);

        if (densityResult.highDensityRegions.isEmpty()) {
            // 没有高密度区域，直接使用标准DBSCAN
            log.debug("未检测到高密度区域，使用标准DBSCAN聚类");
            DBSCANClusterer<TrackPoint> clusterer = new DBSCANClusterer<>(eps, minPts, config.euclideanDistance);
            return clusterer.cluster(points);
        }

        log.debug("密度分析完成：高密度区域 {} 个，总高密度点 {} 个，低密度点 {} 个",
                densityResult.highDensityRegions.size(),
                densityResult.highDensityPoints,
                densityResult.lowDensityPoints);

        // 第二步：分层处理
        List<Cluster<TrackPoint>> allClusters = new ArrayList<>();

        // 处理高密度区域（使用空间抽样优化）
        for (DensityRegion region : densityResult.highDensityRegions) {
            List<Cluster<TrackPoint>> regionClusters = processHighDensityRegionOptimized(region, eps, minPts);
            allClusters.addAll(regionClusters);
        }

        // 处理低密度区域（使用标准DBSCAN）
        if (!densityResult.lowDensityPointsList.isEmpty()) {
            DBSCANClusterer<TrackPoint> clusterer = new DBSCANClusterer<>(eps, minPts, config.euclideanDistance);
            List<Cluster<TrackPoint>> lowDensityClusters = clusterer.cluster(densityResult.lowDensityPointsList);
            allClusters.addAll(lowDensityClusters);
            log.debug("低密度区域聚类完成，得到 {} 个聚类", lowDensityClusters.size());
        }

        log.debug("自适应分层聚类完成，总共得到 {} 个聚类", allClusters.size());
        return allClusters;
    }
     ...
     * 密度分析结果类
     */
    private static class DensityAnalysisResult {
        List<DensityRegion> highDensityRegions = new ArrayList<>();
        List<TrackPoint> lowDensityPointsList = new ArrayList<>();
        int highDensityPoints = 0;
        int lowDensityPoints = 0;
    }

    /**
     * 密度区域类
     */
    private static class DensityRegion {
        ...
        void addPoint(TrackPoint point) {
            points.add(point);
            bounds.expandToInclude(point.getLon(), point.getLat());
        }
    }
     ...
     * 快速密度分析，识别高密度区域
     */
    private DensityAnalysisResult analyzeDensityDistribution(List<TrackPoint> points, double eps, int minPts) {
        DensityAnalysisResult result = new DensityAnalysisResult();

        if (points.size() < 100) {
            // 小数据集直接返回低密度
            result.lowDensityPointsList.addAll(points);
            result.lowDensityPoints = points.size();
            return result;
        }

        // 1. 采样分析（避免处理所有点）
        int sampleSize = Math.min(1000, points.size());
        List<TrackPoint> samplePoints = new ArrayList<>();
        Random random = new Random(42); // 固定种子确保可重复性

        for (int i = 0; i < sampleSize; i++) {
            samplePoints.add(points.get(random.nextInt(points.size())));
        }

        // 2. 计算样本密度分布
        Map<TrackPoint, Integer> neighborCount = new HashMap<>();
        for (TrackPoint point : samplePoints) {
            int count = 0;
            for (TrackPoint other : samplePoints) {
                if (point != other) {
                    double distance = config.euclideanDistance.compute(
                            new double[] { point.getLon(), point.getLat() },
                            new double[] { other.getLon(), other.getLat() });
                    if (distance <= eps) {
                        count++;
                    }
                }
            }
            neighborCount.put(point, count);
        }

        // 3. 确定密度阈值
        List<Integer> counts = new ArrayList<>(neighborCount.values());
        Collections.sort(counts);
        int highDensityThreshold = counts.get((int) (counts.size() * 0.7)); // 前30%为高密度

        // 4. 识别高密度区域
        Set<TrackPoint> highDensitySamplePoints = new HashSet<>();
        for (Map.Entry<TrackPoint, Integer> entry : neighborCount.entrySet()) {
            if (entry.getValue() >= highDensityThreshold) {
                highDensitySamplePoints.add(entry.getKey());
            }
        }

        // 5. 扩展高密度区域
        List<DensityRegion> regions = expandDensityRegions(highDensitySamplePoints, eps);

        // 6. 分配所有点到区域
        Set<TrackPoint> assignedHighDensity = new HashSet<>();
        for (DensityRegion region : regions) {
            for (TrackPoint point : points) {
                if (isPointInRegion(point, region, eps)) {
                    region.addPoint(point);
                    assignedHighDensity.add(point);
                }
            }
            if (region.points.size() >= minPts * 2) { // 高密度区域至少要有2*minPts个点
                result.highDensityRegions.add(region);
                result.highDensityPoints += region.points.size();
            }
        }

        // 7. 剩余点作为低密度处理
        for (TrackPoint point : points) {
            if (!assignedHighDensity.contains(point)) {
                result.lowDensityPointsList.add(point);
                result.lowDensityPoints++;
            }
        }
     ...
     * 扩展密度区域
     */
    private List<DensityRegion> expandDensityRegions(Set<TrackPoint> seedPoints, double eps) {
        List<DensityRegion> regions = new ArrayList<>();
        Set<TrackPoint> processed = new HashSet<>();

        for (TrackPoint seed : seedPoints) {
            if (processed.contains(seed)) {
                continue;
            }

            DensityRegion region = new DensityRegion();
            region.addPoint(seed);
            processed.add(seed);

            // 扩展区域
            expandRegion(region, seedPoints, eps, processed);

            if (region.points.size() >= 3) {
                region.density = region.points.size() / Math.max(region.bounds.getArea(), 0.0001);
                regions.add(region);
            }
        }
     ...
     * 扩展单个区域
     */
    private void expandRegion(DensityRegion region, Set<TrackPoint> candidates, double eps, Set<TrackPoint> processed) {
        List<TrackPoint> toProcess = new ArrayList<>(region.points);

        while (!toProcess.isEmpty()) {
            TrackPoint current = toProcess.remove(0);

            for (TrackPoint candidate : candidates) {
                if (!processed.contains(candidate)) {
                    double distance = config.euclideanDistance.compute(
                            new double[] { current.getLon(), current.getLat() },
                            new double[] { candidate.getLon(), candidate.getLat() });

                    if (distance <= eps * 2) { // 扩展半径稍大一些
                        region.addPoint(candidate);
     ...
     * 判断点是否在区域内
     */
    private boolean isPointInRegion(TrackPoint point, DensityRegion region, double eps) {
        // 快速边界检查
        if (!region.bounds.contains(point.getLon(), point.getLat())) {
            return false;
        }

        // 检查与区域内点的距离
        for (TrackPoint regionPoint : region.points) {
            double distance = config.euclideanDistance.compute(
                    new double[] { point.getLon(), point.getLat() },
                    new double[] { regionPoint.getLon(), regionPoint.getLat() });
            if (distance <= eps * 1.5) {
                return true;
            }
        }
     ...
     * 优化的高密度区域处理（使用空间抽样）
     */
    private List<Cluster<TrackPoint>> processHighDensityRegionOptimized(DensityRegion region, double eps, int minPts) {
        List<TrackPoint> points = region.points;

        log.debug("处理高密度区域，共 {} 个点，密度={}", points.size(), region.density);

        if (points.size() <= 1000) {
            // 小区域直接聚类
            DBSCANClusterer<TrackPoint> clusterer = new DBSCANClusterer<>(eps, minPts, config.euclideanDistance);
            List<Cluster<TrackPoint>> clusters = clusterer.cluster(points);
            log.debug("高密度区域（小）聚类完成，得到 {} 个聚类", clusters.size());
            return clusters;
        }

        // 大区域使用空间抽样策略
        int targetSampleSize = 1000; // 目标样本大小
        double sampleRatio = Math.min(1.0, (double) targetSampleSize / points.size());

        // 空间网格抽样
        List<TrackPoint> sampledPoints = spatialGridSampling(points, sampleRatio, eps);

        log.debug("高密度区域空间抽样：原始 {} 点 -> 抽样 {} 点，抽样比例 {}%",
                points.size(), sampledPoints.size(), sampleRatio);

        // 对抽样数据聚类
        DBSCANClusterer<TrackPoint> clusterer = new DBSCANClusterer<>(eps, minPts, config.euclideanDistance);
        List<Cluster<TrackPoint>> sampleClusters = clusterer.cluster(sampledPoints);

        // 将聚类结果扩展回原数据集
        List<Cluster<TrackPoint>> fullClusters = expandClustersToFullDataset(
                sampleClusters, points, eps);

        log.debug("高密度区域聚类完成，抽样聚类 {} 个 -> 全数据集聚类 {} 个",
                sampleClusters.size(), fullClusters.size());
     ...
     * 空间网格抽样
     */
    private List<TrackPoint> spatialGridSampling(List<TrackPoint> points, double sampleRatio, double eps) {
        // 计算边界
        Envelope bounds = new Envelope();
        for (TrackPoint point : points) {
            bounds.expandToInclude(point.getLon(), point.getLat());
        ...
        // 确定网格大小（基于eps）
        double gridSize = eps * 0.5; // 网格大小为eps的一半
        int gridCols = Math.max(1, (int) Math.ceil(bounds.getWidth() / gridSize));
        int gridRows = Math.max(1, (int) Math.ceil(bounds.getHeight() / gridSize));

        // 将点分配到网格
        Map<String, List<TrackPoint>> gridCells = new HashMap<>();
        for (TrackPoint point : points) {
            int col = (int) Math.min(gridCols - 1, Math.max(0, (point.getLon() - bounds.getMinX()) / gridSize));
            int row = (int) Math.min(gridRows - 1, Math.max(0, (point.getLat() - bounds.getMinY()) / gridSize));
            String key = col + "_" + row;
            gridCells.computeIfAbsent(key, k -> new ArrayList<>()).add(point);
        }

        // 每个网格按比例抽样
        List<TrackPoint> sampledPoints = new ArrayList<>();
        Random random = new Random(42);

        for (List<TrackPoint> cellPoints : gridCells.values()) {
            int cellSampleSize = Math.max(1, (int) Math.ceil(cellPoints.size() * sampleRatio));

            // 随机抽样
            List<TrackPoint> shuffled = new ArrayList<>(cellPoints);
            Collections.shuffle(shuffled, random);

            for (int i = 0; i < Math.min(cellSampleSize, shuffled.size()); i++) {
                sampledPoints.add(shuffled.get(i));
            }
        }
     ...
     * 将抽样聚类结果扩展到全数据集
     */
    private List<Cluster<TrackPoint>> expandClustersToFullDataset(
            List<Cluster<TrackPoint>> sampleClusters, List<TrackPoint> fullPoints, double eps) {

        List<Cluster<TrackPoint>> fullClusters = new ArrayList<>();
        Set<TrackPoint> assignedPoints = new HashSet<>();

        // 为每个抽样聚类创建完整聚类
        for (Cluster<TrackPoint> sampleCluster : sampleClusters) {
            Cluster<TrackPoint> fullCluster = new Cluster<>();

            // 添加抽样聚类中的核心点
            for (TrackPoint samplePoint : sampleCluster.getPoints()) {
                fullCluster.addPoint(samplePoint);
                assignedPoints.add(samplePoint);
            }

            // 扩展聚类到未分配的点
            for (TrackPoint candidate : fullPoints) {
                if (!assignedPoints.contains(candidate)) {
                    // 检查是否与聚类中的点足够近
                    for (TrackPoint clusterPoint : fullCluster.getPoints()) {
                        double distance = config.euclideanDistance.compute(
                                new double[] { candidate.getLon(), candidate.getLat() },
                                new double[] { clusterPoint.getLon(), clusterPoint.getLat() });

                        if (distance <= eps) {
                            fullCluster.addPoint(candidate);
                            assignedPoints.add(candidate);
                            break;
                        }
                    }
                }
            ...
            if (fullCluster.getPoints().size() >= 3) {
                fullClusters.add(fullCluster);
            }
        }
    ...
    public SplitRoadResult splitRoad(List<TrackPoint> wgs84Points, double totalWidthM) {
        ...
        List<Cluster<TrackPoint>> clusters = performAdaptiveHierarchicalClustering(gaussPoints, eps, minPts);
